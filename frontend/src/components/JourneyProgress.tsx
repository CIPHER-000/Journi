  ...
  const pollJobStatus = useCallback(async (jobId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Polling - Job status received:', jobData);
        ...
        if (jobData.status === 'completed') {
          ...
          if (jobData.result?.id) {
            setTimeout(() => {
              navigate(`/journey/${jobData.result.id}`);
            }, 2000);
          }
          return; // Stop polling
        ...
        if (jobData.status === 'processing' || jobData.status === 'queued') {
          pollIntervalRef.current = setTimeout(() => pollJobStatus(jobId), 3000);
        }
      } else {
        console.error('Polling failed:', response.status, response.statusText);
        setError(`Failed to get job status: ${response.status}`);
      }
    } catch (error) {
      console.error('Polling error:', error);
      setError('Connection error during polling');
      // Retry polling after delay
      pollIntervalRef.current = setTimeout(() => pollJobStatus(jobId), 5000);
    }
  }, [onComplete, navigate]);
